...
def f(with_mean,
    ...
    ("linear_regression", Ridge(alpha=alpha, fit_intercept=fit_intercept))
    ])
...
study = optuna.create_study(storage=os.getenv("MY_SQL_CONNECTION_TEST"),
                            study_name="Ridge Regression (MSE)",
                            direction="maximize",
                            load_if_exists=True)
